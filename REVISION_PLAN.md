...
#### Phase 1 status: done 2026-09-19

Section 8, "Evaluation: authoring a schedule from text", is in the manuscript
with Table 2 and Figure 6, both generated by `eval/make_paper_assets.py`
(`make` rebuilds them). Seven models from six vendors: Haiku 4.5, GPT-5.6
Luna, Qwen3.8 Flash and Llama 4 Maverick on all 24 programs; Gemini 3.1
Flash-Lite 20, DeepSeek Flash 16, Sonnet 5 7. Correction to the triage above:
the manuscript already reported the Haiku result in a paragraph at the end of
the MCP section; that paragraph was promoted into the new section, not
written from nothing. Total spend about $12.40 (Haiku $3.30, Sonnet $5.34,
DeepSeek $0.85 real, Gemini $0.52, OpenRouter $2.36 real). The abstract,
contribution 3, conclusion and availability were updated to match, and both
tables now float with `[tbp]`, which also fixes Table 1 printing after the
references. Phases 2 to 6 are untouched.
